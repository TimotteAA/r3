...
export class AvatarService extends BaseService<AvatarEntity, AvatarRepository> {
  ...
  async upload<E extends ObjectLiteral>(data: CreateFileOptions<E>) {
    ...
    const prefix = await this.configure.get<string>("cos.avatarPrefix");
    // 上传到阿里云oss中
    const res = await this.cosService.upload(file, prefix, ossKey);
    ...
    if (!isNil(relation)) {
      ...
      // 到oss中删除
      if (!isNil(oldMedia)) {
        await this.cosService.delete(prefix, oldMedia.key);
        // 删除老的entity
